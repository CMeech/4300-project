...
class StreamSubscriber(DefaultSubscriber):
    ...
    def on_complete(self):
        self._socket.send_payload(
            self._stream_id, Payload(json.dumps('{"COMPLETE": "TRUE"}').encode("utf-8")), complete=True, is_next=False)
        self._requester.mark_completed_and_finish(sent=True)
    ...
    def frame_received(self, frame: Frame):
        if isinstance(frame, CancelFrame):
            ...
            if self.subscriber.subscription is not None:
                ...
                # So this is telling us that the subscription should stem from the publisher
                # but this class doesn't actually attach it... weird
